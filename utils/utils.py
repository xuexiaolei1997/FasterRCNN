...
class DecodeBox():
    ...
    def forward(self, roi_cls_locs, roi_scores, rois, height, width, nms_iou, score_thresh):
        ...
        output = []
        # Using 1 as start means: remove 0 class(background)
        for l in range(1, self.num_classes):
            arg_mask = class_pred == l
            arg_mask = torch.where(arg_mask.cpu())[0].cuda()
            ...
            detections_class = torch.cat(
                [cls_bbox_l, torch.unsqueeze(class_pred[arg_mask] - 1, -1).float(), torch.unsqueeze(class_conf_l, -1)],
                -1)
...
def bbox_iou(bbox_a, bbox_b):
    ...
    # calc area for: Union, box_a, box_b
...
class AnchorTargetCreator(object):
    ...
    def _calc_ious(self, anchor, bbox):
        # iou of anchor and bbox
        # iou's shape: [num_anchors, num_gt]
        ...
        # There is a real box for every pre box corresponding
    ...
    def _create_label(self, anchor, bbox):
        ...
        # -------------------------------------------------------------------------------------------- #
        # argmax_ious
        # define: The serial number of the largest ground truth box corresponding to each a prior box.
        # shape: [num_anchors, ]

        # max_ious
        # define: The iou of the largest real frame corresponding to each real frame.
        # shape: [num_anchors, ]

        # gt_argmax_ious
        # define: The serial number of the largest prior frame corresponding to each real frame.
        # shape: [num_gt, ]
        # -------------------------------------------------------------------------------------------- #
        ...
        # The number of Positive sample will set to 128 if the number is larger than 128
        ...
        # Balance positive/negative sample, keep total number at 256
...
class ProposalTargetCreator(object):
    ...
    def __call__(self, roi, bbox, label, loc_normalize_mean=(0., 0., 0., 0.), loc_normalize_std=(0.1, 0.1, 0.2, 0.2)):
        ...
        # set iou(proposal box and real gt) > neg_iou_thresh_high to positive sample
        # limit number of positive sample in self.pos_roi_per_image
        pos_index = np.where(max_iou >= self.pos_iou_thresh)[0]
        pos_roi_per_this_image = int(min(self.pos_roi_per_image, pos_index.size))
        if pos_index.size > 0:
            pos_index = np.random.choice(pos_index, size=pos_roi_per_this_image, replace=False)

        # set iou(proposal box and real gt) < neg_iou_thresh_low to negative sample
        # keep the sum number of negative and positive sample is self.n_sample
